...
int main(int argc,char** argv){
  ...
  *(sp->compteur)=0;
  ...
  struct servParam*dll_thread_collector=malloc(sizeof(struct servParam));
  //File de message download
  if((k=ftok("./reception",12))==-1){fprintf(stderr,"ftok failed : %s.\n",strerror(errno));};

  /*reset de la file de message*/
  if((dll_thread_collector->msgid=msgget(k,IPC_CREAT | 0666))==-1){fprintf(stderr,"creation fille de message failed : %s.\n",strerror(errno));exit(EXIT_FAILURE);}
  ...
  if((dll_thread_collector->msgid=msgget(k,IPC_CREAT | IPC_EXCL | 0666))==-1){fprintf(stderr,"creation fille de message failed : %s.\n",strerror(errno));exit(EXIT_FAILURE);}

  
  
  dll_thread_collector->compteur=malloc(sizeof(int));
  *(dll_thread_collector->compteur)=0;
  pthread_mutex_init(&(dll_thread_collector->m),NULL);
